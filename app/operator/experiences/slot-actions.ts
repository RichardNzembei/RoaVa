...
export async function addSlots(
  ...
  if (!/^\d{4}-\d{2}-\d{2}$/.test(date) || !/^\d{2}:\d{2}$/.test(time)) {
    return { status: "error", message: t("err_slot_datetime") };
  }
  if (!Number.isInteger(capacity) || capacity < 1 || capacity > 1000) {
    return { status: "error", message: t("err_slot_capacity") };
  }
  if (!Number.isInteger(repeatWeeks) || repeatWeeks < 1 || repeatWeeks > MAX_REPEAT_WEEKS) {
    return {
      status: "error",
      message: t("err_slot_repeat").replace("{n}", String(MAX_REPEAT_WEEKS)),
    };
  }
  ...
  if (overrideRaw) {
    ...
    if (!Number.isInteger(n) || n < 0) {
      return { status: "error", message: t("err_slot_price") };
    }
    priceOverride = n;
  }
  ...
  if (Number.isNaN(base.getTime())) {
    return { status: "error", message: t("err_slot_invalid") };
  }
  if (base.getTime() <= Date.now()) {
    return { status: "error", message: t("err_slot_future") };
  }
  ...
  if (error) {
    return { status: "error", message: t("err_slot_add") };
  }
